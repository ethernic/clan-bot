    ...
    def initialize(self, *options):
        ...
        if options[0].file:
            ...
            if not options[0].gamertag:
                print '["PSN", "Member Since", "Last Played"]'
                for key in sorted(self.clan_dict['Members'].keys(), key=lambda x:x.lower()):
                    self.gamertag = key
    ...
    def display_stats(self):
        ...
        if not self.clan_dict['Members']:
            ...
            try:
                ...
                print '{0} not in Bungie group'.format(self.gamertag.lower())
                print_stats = False
                exit(1)

        if print_stats:
            llist = []
            llist.append(self.gamertag)
            if self.clan_dict['Members']:
                llist.append(self.member_since_days)
                llist.append(member_date)
            llist.append(self.last_played_days)
            llist.append(play_date)
            self.global_list.append(llist)
            with open('pickle.txt', 'wb') as pf:
                pickle.dump(self.global_list, pf)
            
            print '{0}\n'.format(self.gamertag),
            if self.clan_dict['Members']:
                print 'Member Since: {0} days ({1})\n'.format(self.member_since_days, member_date),
            print 'Last Played: {0} days ({1})'.format(self.last_played_days, play_date) 
...
class ClanBot(object):
    ...
    def get_stats(self):
        ...
        #print json.dumps(self.clan_dict, ensure_ascii=False)
        #print len(self.clan_dict['Members'])
>>>>>>> 0a188ecc3171be5f5117e1415b76ae5ccc21e660
        self.display_stats()